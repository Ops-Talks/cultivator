# Contributing to Cultivator

Thank you for your interest in contributing to Cultivator!

Please see the complete Contributing Guide in our documentation: [Contributing Guide](docs/development/contributing.md)

The Contributing Guide includes:

- Ways to contribute (reporting bugs, requesting features, submitting PRs, improving docs)
- Getting started with development
- Pull request checklist
- Code style and testing requirements

## Quick Links

- Full Contributing Guide: [docs/development/contributing.md](docs/development/contributing.md)
- Development Guide: [docs/development/development.md](docs/development/development.md)
- Testing Guide: [docs/development/TESTING.md](docs/development/TESTING.md)
- Architecture: [docs/architecture/design.md](docs/architecture/design.md)
- Report Issues: [GitHub Issues](https://github.com/Ops-Talks/cultivator/issues)
- Discussions: [GitHub Discussions](https://github.com/Ops-Talks/cultivator/discussions)
